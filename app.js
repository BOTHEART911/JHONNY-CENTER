...
function plantillaRender(tpl, t) {
  const v = {
    documento: t.documento || '', nombre: t.nombre || '', residencia: t.residencia || '',
    municipio: t.municipio || '', puesto: t.puesto || '', mesa: t.mesa || '', lider: t.lider || ''
  };
  return String(tpl || '').replace(/\{([a-zA-Z]+)\}/g, (m, k) => (k.toLowerCase() in v) ? v[k.toLowerCase()] : m);
}
...
function sheetConfig() {
  const vars = (VARS.length ? VARS : ['documento', 'nombre', 'residencia', 'municipio', 'puesto', 'mesa', 'lider']);
  openSheet(`<div class="grip"></div>
    <h2 class="h2">Configuración</h2>
    <p class="muted small">Escribe el mensaje que se enviará por WhatsApp. Toca una variable para insertarla donde tengas el cursor.</p>
